    ...
    <div className="space-y-6">
      <div className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
        ...
        <form onSubmit={createProduct} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <select className="w-full rounded-xl border border-stone-300 bg-stone-50 px-4 py-3 text-sm outline-none" value={form.categoryId} onChange={(e) => setForm((p) => ({ ...p, categoryId: e.target.value }))}>
          ...
          <Input placeholder="Cost" type="number" step="0.01" value={form.cost} onChange={(e) => setForm((p) => ({ ...p, cost: e.target.value }))} />
          <Input placeholder="Price" type="number" step="0.01" value={form.price} onChange={(e) => setForm((p) => ({ ...p, price: e.target.value }))} />
          <Input placeholder="Stock Qty" type="number" value={form.stockQty} onChange={(e) => setForm((p) => ({ ...p, stockQty: e.target.value }))} />
          <Input placeholder="Reorder point" type="number" value={form.reorderPoint} onChange={(e) => setForm((p) => ({ ...p, reorderPoint: e.target.value }))} />
            ...
            <tbody>
              {filtered.map((product) => (
                <tr key={product.id} className="border-t border-stone-200">
                  ...
                  <td className="px-3 py-3 text-stone-600">{product.sku ?? '—'} / {product.barcode ?? '—'}</td>
                  <td className="px-3 py-3">{money(product.cost, currencySymbol)}</td>
                  <td className="px-3 py-3">{money(product.price, currencySymbol)}</td>
                  <td className={`px-3 py-3 font-semibold ${product.stockQty <= product.reorderPoint ? 'text-red-600' : 'text-stone-900'}`}>{product.stockQty}</td>
